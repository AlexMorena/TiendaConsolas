    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if ("consola".equals(decision)) {
                ...
                if (miConexion.modificarConsola(potenciaGPU, potenciaCPU, compania, precio, unidadesDisponibles, nombreConsola, idConsola)) {
                    out.println("<h1 style='margin-top: 10px'>Consola modificada correctamente</h1>");
                } else {
                    out.println("<h1 style='margin-top: 10px'>No se ha podido modificar la consola</h1>");
                }
            } else {
                ...
                String precio = request.getParameter("precio");
                String unidadesDisponibles = request.getParameter("unidadesDisponibles");
                String ID_Juego = request.getParameter("idJuego");

                if (miConexion.modificarJuego(nombreJuego, nombreConsola, companiaDesarrolladora, generoJuego, puntMetacritic, precio, unidadesDisponibles, ID_Juego)) {
                    out.println("<h1 style='margin-top: 10px'>Juego modificado correctamente</h1>");
                } else {
                    out.println("<h1 style='margin-top: 10px'>No se ha podido modificar el juego</h1>");
                }
            }
            out.println("<form>");
